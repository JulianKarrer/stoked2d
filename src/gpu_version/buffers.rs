...
use ocl::{
    prm::{Float, Float2, Uchar3, Uint2},
    Buffer, MemFlags, ProQue,
};
...
impl GpuBuffers {
    ...
    pub fn init_gpu_side(
        ...
        self.acc
            .write(&vec![Float2::new(0.0, 0.0); n])
            .enq()
            .unwrap();
        self.handles
            .write(
        ...
        k.reduce_pos_min(n);
        let compute_bdy_cell_keys_kernel = pro_que
            ...
            .unwrap();
        unsafe {
            compute_bdy_cell_keys_kernel.enq().unwrap();
        }

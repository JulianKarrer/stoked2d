...
impl Kernels {
    ...
    pub fn new(b: &GpuBuffers, pro_que: &ProQue, n: u32) -> Self {
        let euler_cromer_kernel = pro_que
            .kernel_builder("eulercromerstep")
            .arg(&b.pos)
            .arg(&b.vel)
            .arg(&b.acc)
            .arg(&b.dt)
            ...
            .build()
            .unwrap();
        let boundary_kernel = pro_que
            .kernel_builder("enforce_boundary")
            .arg(&b.pos)
            .arg(&b.vel)
            .arg(&b.acc)
            .arg(HARD_BOUNDARY[0])
            ...
            .build()
            .unwrap();
        let densitiy_pressure_kernel = pro_que
            .kernel_builder("update_densities_pressures")
            ...
            .build()
            .unwrap();
        let gravity_viscosity_kernel = pro_que
            .kernel_builder("apply_gravity_viscosity")
            ...
            .build()
            .unwrap();
        let pressure_acceleration_kernel = pro_que
            .kernel_builder("add_pressure_acceleration")
            .arg(RHO_ZERO.load(Relaxed) as f32)
            .arg(&b.pos)
            .arg(&b.acc)
            .arg(&b.den)
            ...
            .build()
            .unwrap();
        let compute_neighbours_kernel = pro_que
            .kernel_builder("compute_neighbours")
            .arg(&b.pos_min)
            .arg(&b.pos)
            .arg(&b.handles)
            .arg(&b.neighbours)
            ...
            .build()
            .unwrap();
        let compute_boundary_neighbours_kernel = pro_que
            .kernel_builder("compute_neighbours")
            .arg(&b.pos_min)
            .arg(&b.pos)
            .arg(&b.bdy_handles)
            ...
            .build()
            .unwrap();
        let compute_cell_keys_kernel = pro_que
            .kernel_builder("compute_cell_keys")
            .arg(&b.pos_min)
            .arg(KERNEL_SUPPORT as f32)
            .arg(&b.pos)
            .arg(&b.handles)
            ...
            .build()
            .unwrap();
        let resort_pos_vel = pro_que
            .kernel_builder("resort_data_a")
            .arg(&b.handles)
            .arg(&b.pos)
            .arg(&b.vel)
            .arg(&b.pos_resort)
            ...
            .build()
            .unwrap();
        let resort_pos_vel_b = pro_que
            .kernel_builder("resort_data_b")
            .arg(&b.handles)
            .arg(&b.pos)
            .arg(&b.vel)
            .arg(&b.pos_resort)
        ...
        let radix_a = pro_que
            .kernel_builder("radix_sort_histograms")
            .arg(&b.handles)
            .arg(&b.handles_temp)
            .arg(&b.histograms)
            .arg(&b.counts)
        ...
        let radix_b_small = pro_que
            .kernel_builder("radix_sort_prefixsum_small")
            .arg(&b.histograms)
            ...
            .unwrap();
        let radix_b = pro_que
            .kernel_builder("radix_sort_prefixsum_a")
            .arg(&b.histograms)
            .arg_local::<u32>(WARP)
            .arg(&b.counts)
            .arg(&b.counts_b)
            ...
            .unwrap();
        let radix_c = pro_que
            .kernel_builder("radix_sort_prefixsum_b")
            .arg(&b.histograms)
            .arg(&b.counts_b)
            .arg(n_groups as u32)
            .arg(splinters as u32)
            ...
            .unwrap();
        let radix_d = pro_que
            .kernel_builder("radix_sort_prefixsum_c")
            .arg(&b.histograms)
            .arg(&b.counts_b)
            .arg(n_groups as u32)
            .arg(splinters as u32)
            ...
            .unwrap();
        let radix_e = pro_que
            .kernel_builder("radix_sort_reorder")
            .arg(&b.handles)
            .arg(&b.handles_temp)
            .arg(&b.histograms)
            .arg(&b.counts)
        ...
        let reduce_max_vel = pro_que
            ...
            .build()
            .unwrap();
        let update_dt = pro_que
            .kernel_builder("update_dt")
            .arg(&b.dt)
            .arg(&b.current_t)
            .arg(&b.vel_max)
    ...
    pub fn update_atomics(&self) {
        self.densitiy_pressure
            .set_arg(0, K.load(Relaxed) as f32)
            .unwrap();
        self.densitiy_pressure
            .set_arg(1, RHO_ZERO.load(Relaxed) as f32)
            .unwrap();
        self.gravity_viscosity
            .set_arg(0, NU.load(Relaxed) as f32)
            .unwrap();
        self.gravity_viscosity
            .set_arg(1, GRAVITY.load(Relaxed) as f32)
            .unwrap();
        self.pressure_acceleration
            .set_arg(0, RHO_ZERO.load(Relaxed) as f32)
            .unwrap();
        self.reduce_min_vel_compute_dt.group[1]
            .set_arg(3, LAMBDA.load(Relaxed) as f32)
            .unwrap();
        self.reduce_min_vel_compute_dt.group[1]
    ...
    pub fn update_dt_reduce_vel(&self, b: &GpuBuffers) {
        ...
        self.reduce_min_vel_compute_dt.group[0]
            .set_arg(4, 1u32)
            .unwrap();
        while cur_size > 1 {
            self.reduce_min_vel_compute_dt.group[0]
                .set_arg(3, cur_size as u32)
                .unwrap();
            unsafe {
                self.reduce_min_vel_compute_dt.group[0]
                    .cmd()
